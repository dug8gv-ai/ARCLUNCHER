  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-black text-[var(--text-primary)] tracking-tight">ARC GLOBAL DEVELOPER GUIDE</h2>
        <p className="text-[var(--text-secondary)] mt-2">SDK-Lite for the V4 Precise Ultra-Low Fee Vault Engine</p>
      </div>

      {/* Contract Info */}
      <div className="card rounded-[32px] p-8 shadow-sm border border-[var(--border-dim)]">
        <div className="flex items-center gap-3 mb-6">
          <div className="p-2.5 bg-[rgba(0,242,254,0.05)] text-[var(--accent-cyan)] rounded-2xl">
            <Shield className="w-6 h-6" />
          </div>
          <div>
            <h3 className="text-xl font-bold text-[var(--text-primary)]">Public Vault Contract</h3>
            <p className="text-sm text-[var(--text-secondary)]">Deploy integrations on Arc Testnet</p>
          </div>
        </div>
        ...
        <div className="bg-[var(--bg-card)] border border-[var(--border-dim)] rounded-2xl p-4 flex items-center justify-between">
          ...
          >
            {copiedSection === 'address' ? <CheckCircle className="w-4 h-4 text-emerald-500" /> : <Copy className="w-4 h-4 text-[var(--text-secondary)]" />}
          </button>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
          <div className="bg-slate-50 rounded-2xl p-4 border border-[var(--border-dim)]">
            <p className="font-bold text-[var(--text-primary)] mb-1">Fixed Fee Swap Engine</p>
            <p className="text-sm text-[var(--text-secondary)]">A flat 0.1 fee is charged per swap (USDC/EURC/cirBTC equivalents). All fees automatically settle to the treasury.</p>
          </div>
          <div className="bg-slate-50 rounded-2xl p-4 border border-[var(--border-dim)]">
            <p className="font-bold text-[var(--text-primary)] mb-1">Single-Hop Atomic Swaps</p>
            <p className="text-sm text-[var(--text-secondary)]">Bypass multi-hop routers completely. Swaps are executed instantly against vault liquidity reserves.</p>
          </div>
        </div>
      </div>
      ...
      <div className="card rounded-[32px] p-8 shadow-sm border border-[var(--border-dim)]">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h3 className="text-xl font-bold text-[var(--text-primary)]">Integration Snippets</h3>
        </div>
