      ...
      <div className="bg-white border border-slate-200/80 rounded-[32px] p-6 sm:p-8 shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5 mb-6">
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center bg-slate-100 p-1 rounded-xl">
              <button onClick={() => setActiveTab('board')} className={`px-4 py-2 rounded-lg text-xs font-black transition-all ${activeTab === 'board' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Gigs Board</button>
              <button onClick={() => setActiveTab('my_gigs')} className={`relative px-4 py-2 rounded-lg text-xs font-black transition-all ${activeTab === 'my_gigs' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                My Gigs
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
            <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => { setIsPosting(false); setNewImage(null); setIsProposal(false); setProposalImage1(null); setProposalImage2(null); }} className="text-slate-400 hover:text-slate-600 p-1.5 rounded-full hover:bg-slate-100"><X size={18} /></button>
              </div>
              <form onSubmit={handlePostGig} className="space-y-4">
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Cover Image (Optional)</label>
                  <input ref={fileInputRef} type="file" accept="image/*" onChange={handleImageSelect} className="hidden" />
                  {newImage ? (
                    <div className="relative rounded-xl overflow-hidden border border-slate-200">
                      <img src={newImage} alt="Preview" className="w-full h-36 object-cover" />
                      <button type="button" onClick={() => { setNewImage(null); if (fileInputRef.current) fileInputRef.current.value = ''; }} className="absolute top-2 right-2 bg-white/90 text-slate-600 p-1.5 rounded-full shadow-sm"><X size={14} /></button>
                    </div>
                  ) : (
                ...
                <div className="border border-slate-200 rounded-2xl p-4 bg-slate-50/50">
                  <div className="flex items-center justify-between cursor-pointer" onClick={() => { setIsProposal(!isProposal); setProposalImage1(null); setProposalImage2(null); }}>
                    <div><p className="text-xs font-black text-slate-700">Add Proposal Samples</p><p className="text-[10px] text-slate-400 mt-0.5">Attach up to 2 portfolio images</p></div>
                    <div className={`relative w-11 h-6 rounded-full transition-all flex-shrink-0 ${isProposal ? 'bg-indigo-600' : 'bg-slate-300'}`}><div className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${isProposal ? 'left-5' : 'left-0.5'}`} /></div>
                  </div>
                  {isProposal && (
                    <div className="mt-4 grid grid-cols-2 gap-3">
                      {[{ ref: proposalRef1, img: proposalImage1, set: setProposalImage1, label: 'Sample 1' }, { ref: proposalRef2, img: proposalImage2, set: setProposalImage2, label: 'Sample 2' }].map((item, i) => (
                        <div key={i}>
                          <p className="text-[10px] font-bold text-slate-400 uppercase mb-1.5">{item.label}</p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl animate-in zoom-in-95 duration-200">
              <div className="text-center mb-5">
                ...
                <p className="text-xs text-slate-500 mt-1">Release funds to the freelancer.</p>
              </div>
              <div className="space-y-3 bg-slate-50 border border-slate-200 rounded-2xl p-4 mb-5 text-xs">
                <div className="flex justify-between"><span className="text-slate-400 font-bold uppercase text-[10px]">Gig</span><span className="font-black text-slate-700 truncate max-w-[180px]">{payConfirmGig.title}</span></div>
                <div className="w-full h-px bg-slate-200" />
                <div className="flex justify-between"><span className="text-slate-400 font-bold uppercase text-[10px]">Amount</span><span className="font-black text-emerald-600 text-base">${payConfirmGig.budget} USDC</span></div>
                <div className="w-full h-px bg-slate-200" />
                <div className="flex justify-between items-center"><span className="text-slate-400 font-bold uppercase text-[10px]">To</span>
                  <div className="flex items-center gap-2"><img src={payConfirmGig.freelancerProfile?.avatar} alt="" className="w-6 h-6 rounded-full border border-slate-200" /><span className="font-black text-slate-700">{payConfirmGig.freelancerProfile?.name}</span></div>
                </div>
                <div className="w-full h-px bg-slate-200" />
                <div className="flex justify-between"><span className="text-slate-400 font-bold uppercase text-[10px]">Wallet</span><span className="font-mono text-slate-500 text-[10px]">{payConfirmGig.freelancer_wallet?.slice(0, 8)}...{payConfirmGig.freelancer_wallet?.slice(-6)}</span></div>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl animate-in zoom-in-95 duration-200">
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-lg font-black text-slate-800">Send USDC</h3>
                <button onClick={() => { setDirectPayTarget(null); setDirectPayAmount(''); }} className="text-slate-400 hover:text-slate-600 p-1.5 rounded-full hover:bg-slate-100"><X size={18} /></button>
              </div>
              <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-2xl p-4 mb-4">
                <img src={directPayTarget.avatar} alt="" className="w-12 h-12 rounded-full border-2 border-white shadow" />
                <div><p className="font-black text-slate-800">{directPayTarget.name}</p><p className="text-[10px] font-mono text-slate-400">{directPayTarget.wallet.slice(0, 10)}...{directPayTarget.wallet.slice(-6)}</p></div>
              </div>
              <div className="mb-4">
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Amount (USDC)</label>
                <div className="relative">
                  <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                  <input type="number" min="0.01" step="0.01" value={directPayAmount} onChange={e => setDirectPayAmount(e.target.value)} placeholder="10.00" className="w-full pl-9 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-black outline-none focus:border-indigo-500" />
                </div>
              </div>
              <div className="flex gap-3">
                <button onClick={() => { setDirectPayTarget(null); setDirectPayAmount(''); }} className="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-xs font-black">Cancel</button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl animate-in zoom-in-95 duration-200">
              <div className="flex items-center justify-between mb-5">
                ...
                <button onClick={() => setProfileModal(null)} className="text-slate-400 hover:text-slate-600 p-1.5 rounded-full hover:bg-slate-100"><X size={18} /></button>
              </div>
              <div className="text-center mb-5">
                <img src={profileModal.avatar} alt="" className="w-20 h-20 rounded-full border-4 border-indigo-100 mx-auto mb-3 shadow-lg" />
                <h4 className="text-xl font-black text-slate-900">{profileModal.name}</h4>
                {profileModal.bio && <p className="text-xs text-slate-500 mt-1 leading-relaxed">{profileModal.bio}</p>}
                <p className="text-[10px] font-mono text-slate-400 mt-2 bg-slate-50 rounded-lg px-3 py-1.5 inline-block">{profileModal.wallet}</p>
              </div>
        ...
        {chatGig && (
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/40 backdrop-blur-sm p-0 sm:p-4">
            <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full sm:max-w-lg shadow-2xl animate-in slide-in-from-bottom duration-200 flex flex-col h-[85vh] sm:h-[75vh]">
              {/* Chat Header */}
              <div className="flex items-center justify-between p-4 border-b border-slate-100 flex-shrink-0">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => { setChatGig(null); setChatMessages([]); }} className="text-slate-400 hover:text-slate-600 p-2 rounded-full hover:bg-slate-100"><X size={18} /></button>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
                {chatLoading ? (
                  ...
                  <div className="text-center py-10"><MessageCircle size={28} className="text-slate-200 mx-auto mb-2" /><p className="text-xs text-slate-400 font-bold">No messages yet. Start the conversation!</p></div>
                ) : (
                    ...
                    return (
                      <div key={msg.id} className={`flex gap-2.5 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                        <button onClick={() => setProfileModal({ ...msg.senderProfile, wallet: msg.sender_wallet })} className="flex-shrink-0 mt-1 hover:opacity-80 transition-opacity">
                          <img src={msg.senderProfile?.avatar} alt="" className="w-8 h-8 rounded-full border border-slate-200" />
                        </button>
                        <div className={`max-w-[75%] flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                          <button onClick={() => setProfileModal({ ...msg.senderProfile, wallet: msg.sender_wallet })} className={`text-[10px] font-bold mb-0.5 hover:underline ${isMe ? 'text-indigo-500' : 'text-slate-400'}`}>
              ...
              <form onSubmit={handleSendChat} className="p-3 border-t border-slate-100 flex gap-2 flex-shrink-0">
                <input value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder="Type a message..." className="flex-1 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs outline-none focus:border-indigo-500" />
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    ...
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <div className="flex flex-col items-end gap-1 flex-shrink-0">
                        ...
                        <span className={`px-2.5 py-1 text-[9px] font-black uppercase tracking-wider rounded-lg ${gig.status === 'OPEN' ? 'bg-emerald-100 text-emerald-600' : gig.status === 'IN_PROGRESS' ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-600'}`}>
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-100 space-y-2">
                      ...
                      {gig.status === 'IN_PROGRESS' && gig.freelancer_wallet === userAddress?.toLowerCase() && (
                        <div className="flex items-center justify-center gap-2 py-2.5 bg-blue-50 text-blue-600 rounded-xl border border-blue-100 text-xs font-black"><Clock size={14} /> You are working on this</div>
                      )}
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => openChat(gig)} className="flex-1 py-2.5 bg-slate-50 hover:bg-slate-100 text-slate-600 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-2 border border-slate-200">
